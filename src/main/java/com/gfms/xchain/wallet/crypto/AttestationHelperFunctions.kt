package com.gfms.xchain.wallet.crypto

import com.gfms.xnet.utils.sha256AsBigInt
import com.gfms.xnet.utils.sha256_4_AsBigInt
import com.gfms.xnet.utils.sha512AsBigInt
...
fun attestSHA512(publicKey: BonehPublickKey, value: ByteArray): BonehAttestation {
...
fun createHonestyCheck(publicKey: BonehPublickkey, value: Int): FP2Value {
...
fun attest(publicKey: BonehPublicKey, value:BigInteger, bitSpace: Int): BonehAttestation {
    ...
    val tOutPrivate = arrayListOf<Pair<Int, FP2CValue>>()
    ...
    tOutPublicShuffled.forEach {
        shuffleMap[it.first] = outPrivate.size
    ...
    outPrivate.forEach {
        hitPairs.add(BitPairAttestation(outPublic[it.first], outPublic[it.first + 1], it.second))
    }
...
fun binaryRelateiveCertainty(expected: HashMap<Int, Int>, value: HashMap<Int, Int>): Double {
    val exp = value.values.sumOf { it }
    val cert = 1.0 - 0.5.pow(exp)
    return binaryRelativityMatch(expected, value) * cert
}

fun binaryRelativeMatch(expected: HashMap<Int, Int>, value: HashMap<Int, Int>): Double {
    var match = 1.0
...
fun createEmptyRelativityMap(): HashMap<Int, Int> {
    return hashMapOf(0 to 1, 1 to 0, 2 to 0, 3 to 0)
}
